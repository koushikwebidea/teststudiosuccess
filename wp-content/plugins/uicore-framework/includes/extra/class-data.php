...
class Data
{
    ...
    {
        if (false === ($data = get_transient('uicore_library_v2_' . $type))) {
            $type = ($type === 'pages') ? $type.'/'. strtolower(UICORE_NAME) : $type;
            $data = [];
            $data = wp_remote_get(UICORE_LIBRARY . $type);
            $data = wp_remote_retrieve_body($data);
            set_transient('uicore_library_v2_' . $type, $data, 7 * 2);
        }
        return $data;
    }
    ...
    static function get_menu_icons($name=false)
    {
        //Return markup for one
        if($name){
            return file_get_contents(UICORE_PATH . '/assets/img/menu-icons/'.$name);
        }
